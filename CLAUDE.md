...
**PictureFramer** — an iOS SwiftUI app (iPhone + iPad, portrait and landscape) that imports photos of framed paintings/pictures from the photo library or shoots them in-app with an AVFoundation camera view (capture feeds the same `load(data:)` funnel; original never saved), auto-detects the outer edge of the artwork including its frame (Vision), perspective-corrects it so it appears perfectly straight (Core Image), adds a user-configurable pixel margin of *real background* equally on all four sides, and exports the result to the photo library. A crop-mode toggle (picker screen + editor, persisted in UserDefaults) switches between this framed-with-wall crop and a painting-only crop that targets the painting inside the frame (nested-rectangle detection with outer-quad fallback) and forces the margin to zero. An optional AI step removes glass reflections: an on-device heuristic proposes a glare mask, the user refines it with a brush, and a cloud inpainting provider (OpenAI or Gemini, user's own API key) repaints only the masked pixels.
...
- **Adaptive layout** (`Core/Layout/CanvasShape.swift`, `Core/Layout/EditorLayout.swift`, rendered by `UI/AdaptiveEditorLayout.swift`): every screen arranges itself by canvas *shape* — wider than tall → side by side, else stacked — never by interface orientation (iPhone Duo's inner display and iPad multitasking don't report it reliably). Editors put the controls in a side column (≤380 pt; 42% of width, 50% on canvases under 500 pt tall so phone-landscape labels stay single-line) with the primary action row pinned under a scrolling controls stack, so Save is always visible. The picker screen puts the hero beside the buttons on wide canvases. All landscape orientations are enabled for iPhone and iPad (`project.yml`).
- **Camera** (`UI/Camera/CameraSession.swift` + `CameraCaptureView.swift`): AVFoundation capture, not `UIImagePickerController` (whose camera UI is portrait-only). `AVCaptureDevice.RotationCoordinator` rotates the preview and stamps the capture's rotation so landscape shots come out upright; output is JPEG data into `EditorViewModel.loadCapturedPhoto`. The shutter bar sits under the preview on tall canvases and beside it on wide ones. Simulator has no camera — the Take Photo button is hidden there — so capture is only verifiable on hardware (TestFlight); the launch argument `-ShowCameraWithoutHardware` forces the button for UI tests (session never configures). The capture view switches its arrangement with `AnyLayout`, never with separate `VStack`/`HStack` branches: a branch change gives `CameraPreview` a new identity, SwiftUI destroys the `UIView` whose layer is the session's preview layer, and on device the preview goes black and the app can crash when that layer deallocates under the running session (`testCameraViewSurvivesRotation` guards this).
...
- Appearance (System/Light/Dark, `Core/Config/AppAppearance.swift`, key `appAppearance`) is applied via `UIWindow.overrideUserInterfaceStyle` (`UI/AppearanceApplier.swift`) from the Settings picker's `onChange` and once at launch — NOT via `preferredColorScheme` and NOT via a root-level `@AppStorage` observer: both were tested and silently missed the second change made from inside the Settings sheet.
