...
- **Editor layout** (`Core/Layout/EditorLayout.swift`, rendered by `UI/AdaptiveEditorLayout.swift`): image area + controls stack on phones; on a regular-width canvas that is wider than tall (iPad, iPhone Duo inner display in landscape, wide Split View) the controls move into a side column (≤380 pt / ≤42% width) so the picture keeps its height. Decided from size class + geometry only, never from interface orientation (Apple's iPhone Duo guidance: the inner display ignores `UISupportedInterfaceOrientations`). Both `EditorView` and `ReflectionEditView` use it.
